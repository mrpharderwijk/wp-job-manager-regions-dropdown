<?php
 /*
  * this file is located in wp-content/plugins/wp-job-manager/includes/class-wp-job-manager-ajax.php
  */
...
class WP_Job_Manager_Ajax {
	...
	public function get_listings() {
		...
		$search_region      = sanitize_text_field( stripslashes( $_POST['search_region'] ) );
		...
		if ( sizeof( $filter_job_types ) > 0 && ( sizeof( $filter_job_types ) !== sizeof( $types ) || $search_keywords || $search_location || $search_region || $search_categories || apply_filters( 'job_manager_get_listings_custom_filter', false ) ) ) {
			$showing_types = array();
			$unmatched     = false;
			...
			$showing_region    = $search_region ? sprintf( ' ' . __( 'located in &ldquo;%s&rdquo;', 'wp-job-manager' ), $search_region ) : '';

			$result['showing'] = apply_filters( 'job_manager_get_listings_custom_filter_text', $showing_jobs . $showing_location . $showing_region);
