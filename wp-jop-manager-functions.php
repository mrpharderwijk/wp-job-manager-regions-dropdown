<?php
/*
 * this file is located in wp-content/plugins/wp-job-manager/wp-job-manager-functions.php
 */
...
function get_job_listings( $args = array() ) {
	...
	// Region search - search joblistings according to job_region
	if ( $args['search_region'] ) {
		$region_post_ids = array_merge( $wpdb->get_col( $wpdb->prepare( "
		    SELECT DISTINCT post_id FROM {$wpdb->postmeta}
		    WHERE post_id IN (
		...
		", $args['search_region'] ) ), array( 0 ) );
	} else {
		$region_post_ids = array();
	}
	...
	// Merge post ids	
	if ( empty( $location_post_ids ) || $location_post_ids < 1 ) {
		$query_args['post__in'] = array_merge( $region_post_ids );
	} elseif ( ! empty( $location_post_ids ) && ! empty( $keyword_post_ids ) && ! empty( $region_post_ids ) ) {
		$query_args['post__in'] = array_intersect( $location_post_ids, $keyword_post_ids, $region_post_ids );
	} elseif ( ! empty( $location_post_ids ) || ! empty( $keyword_post_ids ) || ! empty( $region_post_ids ) ) {
		$query_args['post__in'] = array_merge( $location_post_ids, $keyword_post_ids, $region_post_ids );
	}
