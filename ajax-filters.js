/*
 * this file is located in wp-content/plugins/wp-job-manager/assets/js/ajax-filters.js
 */
jQuery( document ).ready( function ( $ ) {
	...
	$( '.job_listings' ).on( 'update_results', function ( event, page, append ) {
		...
		if ( true == target.data( 'show_filters' ) ) {
			...
			var $region 	= form.find( 'select[name=search_region]' );
	...
	$( '#search_keywords, #search_location, .job_types input, #search_categories, #search_region' ).change( function () {
	...
	$( '.job_filters' ).each(function() {
		$( this ).find( '#search_keywords, #search_location, .job_types input, #search_categories, #search_region' ).eq(0).change();
	});
	...
	$( '.job_filters' ).on( 'click', '.reset', function () {
		...
		form.find( 'select[name=search_region]' ).val( '' );
